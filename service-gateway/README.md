...
## 简介
`service-gateway` 是基于 **Spring Cloud Gateway** 构建的 API 网关。它是微服务架构的流量入口和安全的第一道防线。
...
2.  **统一鉴权 (Resource Server)**:
    ...
    *   通过配置的 `jwk-set-uri` (指向 `service-auth`)，拉取公钥并**校验 JWT Token 的签名和有效期**。
...
- 示例：`http://localhost:9527/service-auth/**` -> 转发到 `service-auth` 服务
...
在 [SecurityConfig.java](file:///root/code/java/gitcode/springcloud2024/service-gateway/src/main/java/com/overthinker/cloud/gateway/config/SecurityConfig.java) 中配置了以下路径为 `permitAll()`：

| 路径 | 说明 |
| :--- | :--- |
| `/actuator/**` | 健康检查和监控端点 |
| `/service-auth/**` | 认证服务的所有端点（用户登录、获取Token等） |
...
*   **端口**: `9527`
*   **Issuer URI**: `http://localhost:9123` (service-auth 的 Issuer URI)
*   **JWK URI**: `http://localhost:9123/oauth2/jwks` (必须能访问到 service-auth)
*   **配置文件**: 

...
`service-web` 是 `springcloud2024` 项目的**核心业务后端服务**，专注于博客/资讯类业务功能。它提供文章、视频、评论、留言、相册、友链、轮播图、点赞、收藏、树洞、菜单、网站信息、服务器监控等完整的 RESTful API，并作为面向用户交互的主要入口点。

该模块是一个综合性的 Spring Boot 应用，集成了项目内部的各种服务和启动器。它处理用户请求、执行业务逻辑、与数据库交互，并通过 Consul 服务发现注册到微服务体系中，由 `service-gateway` 网关统一路由转发。

> **关于角色与权限**：原 `Role`、`RoleMenu`、`ChatGpt` 等实体及对应业务已迁移至 `service-auth` 服务，本模块不再承担 RBAC 鉴权职责。菜单数据由 `service-auth` 提供。
...
> **注意**：`Server`、`Menu` 等类不在此模块维护。`Server` 为 OSHI 监控 VO；`Menu` 数据已迁移至 `service-auth`。

## 5. 配置

作为核心业务模块，`service-web` 需要配置多个外部服务。配置统一通过 Consul 或 `application.yaml` 提供：
...
具体属性键请参考 `application.yaml` 与 Nacos/Consul 上的 `service-web.yaml`。
...
- 原 `t_chat_gpt`、`sys_role`、`sys_role_menu` 等表已迁移至 `service-auth`
...
mvn -pl service-web -am clean package -DskipTests
java -jar service-web/target/service-web.jar
```
...
java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005 \
     -jar service-web/target/service-web.jar
```

### 8.3 Docker 启动

`service-web` 根目录已提供 `Dockerfile`，可执行：

```bash
docker build -t service-web .
docker run -p 8080:8080 service-web
```
...
- 菜单、角色、权限相关接口已迁出，前端菜单与权限请统一调用 `service-auth` 服务（`/service-auth/menu`、`/service-auth/role`）。
- `service-web` 注册到 Consul 后，外部请求应通过 `service-gateway` 网关，路径前缀为 `/service-web`。
